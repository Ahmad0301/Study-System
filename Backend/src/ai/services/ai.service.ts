...
${documentText}`;
    ...
    const replyText = await this.openRouterService.chatCompletion(messages, 0.4);
...
[
  ...
  async sendMessageToSession(userId: string, sessionId: string, dto: SendSessionMessageDto) {
    ...
    const systemPrompt = `You are AI Study Assistant, an expert tutor and interactive coding assistant. Provide clear, accurate, markdown-formatted responses. Use clean code blocks with language identifiers when writing code.${
    ...
    const aiResponseText = await this.openRouterService.chatCompletion(conversationHistory, 0.7);
