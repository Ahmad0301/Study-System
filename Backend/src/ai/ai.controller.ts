...
export class AiController {
  ...
  @Get('general-chat')
  getGeneralChatSession(@Req() req: Request) {
    const userId = (req as any).user.sub;
    return this.aiService.getGeneralChatSession(userId);
  }

  @Post('general-chat/message')
  @HttpCode(HttpStatus.OK)
  sendGeneralChatMessage(@Req() req: Request, @Body() dto: SendGeneralChatMessageDto) {
    const userId = (req as any).user.sub;
    return this.aiService.sendGeneralChatMessage(userId, dto);
  }

  @Delete('general-chat')
  clearGeneralChatSession(@Req() req: Request) {
    const userId = (req as any).user.sub;
