...
export default function DashboardPage() {
  ...
  const [weeklyStats, setWeeklyStats] = useState<{ days: any[]; totalHours: number }>({
    days: [],
    totalHours: 0,
  });
  ...
  useEffect(() => {
    let isMounted = true;
    Promise.all([
      subjectService.getDashboardStats(),
      subjectService.getAll(),
      subjectService.getRecentActivities(),
      activityService.getWeeklyStats().catch(() => null),
    ])
      .then(([statsData, subjectsData, activitiesData, weeklyData]) => {
        if (!isMounted) return;
        if (statsData) setLiveStats(statsData);
        if (Array.isArray(subjectsData)) setRealSubjects(subjectsData);
        if (Array.isArray(activitiesData)) setRealActivities(activitiesData);
  ...
  if (loading) {
    return <GlobalPageLoader message="Loading dashboard statistics…" />;
  }
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 p-6 flex flex-col justify-between">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2 text-xs font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/50 px-2.5 py-1 rounded-lg">
              <TrendingUp size={14} /> {weeklyStats.totalHours} hrs total
            </div>
          </div>

          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={weeklyStats.days.length > 0 ? weeklyStats.days : mockWeeklyStudyStats} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                <defs>
