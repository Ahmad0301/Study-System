...
export default function SubjectsPage() {
  ...
  const validate = () => {
    ...
    if (!form.name.trim()) e.name = "Subject name is required.";
      ...
      e.name = "Subject name must not exceed 30 characters.";

    if (!form.description.trim()) e.description = "Description is required.";
    else if (form.description.trim().length > 150)
      e.description = "Description must not exceed 150 characters.";
    return e;
  };
  ...
  if (initialLoading) {
    return <GlobalPageLoader message="Loading your subjects…" />;
  }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center p-1 rounded-xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 text-slate-400 shadow-xs">
          <button
      ...
      {/* Subjects Grid/List or Empty State */}
      {filteredSubjects.length === 0 ? (
          ...
          {filteredSubjects.map((s) => (
            ...
            >
              ...
              <div>
                ...
                <h3 className="font-bold text-slate-900 dark:text-slate-100 text-base mb-1.5 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors break-words">
                  {s.name}
                </h3>
                <p className="text-xs text-slate-500 dark:text-slate-400 whitespace-pre-line break-words line-clamp-3 leading-relaxed mb-6">
                  {s.description}
                </p>
              </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 divide-y divide-slate-100 dark:divide-slate-800/60 overflow-hidden shadow-sm">
          {filteredSubjects.map((s) => (
            <div
              key={s.id}
              className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors group cursor-pointer"
              onClick={() => router.push(`/dashboard/subjects/${s.id}`)}
            >
              <div className="flex items-start gap-4 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="font-bold text-slate-900 dark:text-slate-100 text-base group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors break-words">
                    {s.name}
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 whitespace-pre-line break-words leading-relaxed mt-1">
                    {s.description}
                  </p>
              ...
              <div className="flex items-center gap-6 shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100 dark:border-slate-800">
                <div className="w-32 space-y-1.5">
                  <div className="flex justify-between text-xs">
                    <span className="text-[10px] text-slate-400 uppercase font-bold">Mastery</span>
                    <span className="font-bold text-slate-700 dark:text-slate-300 text-xs">{s.progress}%</span>
                  </div>
                  <div className="h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                    <div
                      className="h-full rounded-full transition-all duration-500"
                ...
                <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                  ...
                  <Link
                    href={`/dashboard/subjects/${s.id}`}
                    className="p-2 rounded-xl bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900 transition-colors ml-1"
                    title="View Subject"
                  >
            ...
            <form onSubmit={handleSubmit} noValidate className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className={`w-full px-4 py-2.5 rounded-xl border text-xs bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 focus:bg-white dark:focus:bg-slate-900 outline-none transition-all resize-y ${
                    errors.description
                      ? "border-red-500 focus:ring-2 focus:ring-red-500/20"
                      : "border-slate-200 dark:border-slate-800 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
                  }`}
