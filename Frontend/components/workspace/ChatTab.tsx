...
function MarkdownText({ content, onSelectPrompt }: { content: string; onSelectPrompt?: (prompt: string) => void }) {
  const { mainText, questions } = parseFollowUpQuestions(content);
  ...
  return (
    <div className="space-y-3 text-xs sm:text-sm leading-relaxed">
      {parts.map((part, index) => {
        ...
        return <RenderWorkspaceBlocks key={index} text={part} />;
      })}

      {questions.length > 0 && (
        <div className="pt-3 mt-4 border-t border-slate-200 dark:border-slate-800 space-y-2">
          <p className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
            <Lightbulb size={14} className="text-amber-500" /> Suggested Follow-ups
          </p>
          <div className="flex flex-wrap gap-2">
            {questions.map((q, qIdx) => (
              <button
                ...
                className="text-left text-xs font-medium px-3 py-1.5 rounded-xl bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-800 hover:bg-blue-100 dark:hover:bg-blue-900 transition-all hover:scale-[1.01] active:scale-95"
              >
                💡 {q}
              </button>
            ))}
...
function RenderWorkspaceBlocks({ text }: { text: string }) {
  ...
  const flushTable = (key: number) => {
    if (tableRows.length > 0) {
      elements.push(<RenderWorkspaceTable key={`wtable-${key}`} rows={tableRows} />);
      tableRows = [];
    }
    inTable = false;
  };
  ...
  lines.forEach((line, idx) => {
    ...
    if (!trimmed) {
      elements.push(<div key={`wempty-${idx}`} className="h-1.5" />);
      return;
    }
    ...
    if (/^[\-\*\_]{3,}$/.test(trimmed)) {
      elements.push(
        <hr key={idx} className="my-3 border-t border-slate-200 dark:border-slate-800" />
      );
      return;
    }

    // Headings: match 1 to 6 leading # symbols
    ...
    if (headingMatch) {
      ...
      if (level === 1) {
        elements.push(
          <h2 key={idx} className="font-black text-lg sm:text-xl text-slate-900 dark:text-slate-100 mt-5 mb-2">
            {formatFormattedText(titleText)}
          </h2>
        );
      } else if (level === 2) {
        elements.push(
          <h3 key={idx} className="font-extrabold text-base sm:text-lg text-slate-900 dark:text-slate-100 mt-4 mb-1.5">
            {formatFormattedText(titleText)}
          </h3>
        );
      } else {
        elements.push(
          <h4 key={idx} className="font-bold text-sm sm:text-base text-slate-900 dark:text-slate-100 mt-4 mb-1">
            {formatFormattedText(titleText)}
          </h4>
        );
      }
    ...
    if (trimmed.startsWith("> ")) {
      elements.push(
        <blockquote key={idx} className="border-l-4 border-blue-500 pl-3.5 py-1.5 my-2 bg-blue-50/50 dark:bg-blue-950/30 text-slate-700 dark:text-slate-300 italic text-xs sm:text-sm rounded-r-xl">
          {formatFormattedText(trimmed.slice(2))}
        </blockquote>
      );
      return;
    }
    ...
    if (orderedMatch) {
      ...
      elements.push(
        <div key={idx} className="flex items-start gap-2 ml-2 my-1">
          <span className="text-xs font-bold text-blue-600 dark:text-blue-400 shrink-0 font-mono">{num}.</span>
          <span>{formatFormattedText(listContent)}</span>
        </div>
      );
      return;
...
function RenderWorkspaceTable({ rows }: { rows: string[] }) {
    ...
    <div className="my-3 overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
      <table className="w-full text-left text-xs sm:text-sm border-collapse">
        <thead className="bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-slate-100 font-bold border-b border-slate-200 dark:border-slate-800">
          <tr>
            {headerCells.map((h, i) => (
              <th key={i} className="p-2.5 border-r border-slate-200 dark:border-slate-800 last:border-r-0">
                {formatFormattedText(h)}
              </th>
            ))}
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-100 dark:divide-slate-800/80 bg-white dark:bg-slate-900">
          {bodyRows.map((r, rIdx) => (
            <tr key={rIdx} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
              {r.map((cell, cIdx) => (
                <td key={cIdx} className="p-2.5 border-r border-slate-200 dark:border-slate-800/60 last:border-r-0 text-slate-700 dark:text-slate-300">
                  {formatFormattedText(cell)}
                </td>
              ))}
            </tr>
          ))}
